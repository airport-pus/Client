...
const getCongestionLabel = (value: number): string => {
...
const ParkingCongestion = () => {
  const { data, error } = useSWR<ParkingLot[]>(
  ...
  const maxRemainingLot = data.reduce(
    (maxLot: ParkingLot, lot: ParkingLot) =>
      lot.remainingSpace > maxLot.remainingSpace ? lot : maxLot,
    data[0]
  )

  const bestParkingName = maxRemainingLot.parkingAirportCodeName

  const parkingData: ParkingData[] = data.map((lot: ParkingLot) => ({
    name: lot.parkingAirportCodeName,
    congestion:
      lot.parkingCongestionDegree === "100%" || lot.remainingSpace === 0
        ? "만차"
        : getCongestionLabel(parseFloat(lot.parkingCongestionDegree)),
  ...
  const chartData = {
    labels: parkingData.map((lot: ParkingData) => lot.name),
    datasets: [
      {
        label: "주차장 혼잡도",
        data: parkingData.map((lot: ParkingData) => lot.degree),
        borderColor: "#215DCE",
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (tooltipItem: ExtendedTooltipItem): string[] => {
