...
const flightData = Array.from({ length: 40 }, (_, i) => ({
  id: i + 1,
  airline: "대한항공",
  flightNumber: `KE44${40 + i}`,
  destination: "제주",
  gate: (i % 5) + 1,
  status: i % 3 === 0 ? "지연" : i % 3 === 1 ? "마감 예정" : "정상",
  scheduledTime: `2025-02-12 08:${30 + (i % 10)}`,
  modifiedTime: `2025-02-12 08:${30 + (i % 10) + (i % 3 === 0 ? 5 : 0)}`,
  delay: i % 3 === 0 ? "지연 5분" : "",
}));
  ...
  return (
    <>
      ...
      {flightData.map((flight) => (
        <div key={flight.id} className="grid grid-cols-5 border-b p-4 items-center">
          <div className="flex items-center gap-2 ml-14">
            <img src="/logos/korean-air.webp" alt={flight.airline} className="w-8 h-8" />
            <div>
          ...
          <div className={`text-center ${flight.status === "지연" ? "text-red-500" : "text-blue-500"} ml-2`}>{flight.status}</div>
          <div className="flex flex-col items-start ml-14">
            ...
            {flight.delay && <div className="text-red-500 text-sm">{flight.delay}</div>}
          </div>
        </div>
      ))}
    </>
